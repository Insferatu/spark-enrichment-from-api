docker compose --project-directory infrastructure exec spark-submit bash \
  ...
  --driver-java-options "-Dsample-count=10000 -Dprediction-provider.processing-method.type=restricted-parallel-processing -Dprediction-provider.processing-method.total-parallelism=50" \
